...
//
//	// 这四个点分别位于像素的左下、右下、左上、右上四个象限
//	float offsets[4][2] = { {0.25, 0.25}, {0.75, 0.25}, {0.25, 0.75}, {0.75, 0.75} };
//
//
...
//		{
//			// 用于累加颜色的变量
//			Eigen::Vector3f color_accum(0, 0, 0);
//			int inside_count = 0; // 记录该像素内有多少个采样点在三角形内
//
//			// --- 4. 遍历该像素内的 4 个采样点 ---
//			for (int k = 0; k < 4; k++) {
//				float sx = x + offsets[k][0];
//				float sy = y + offsets[k][1];
//
//
...
//					int depth_buf_idx = pixel_idx * 4 + k; // 每个像素占4个深度槽位
//					//深度缓存机制
//					if (zp < depth_buf[depth_buf_idx])
//					{
//						depth_buf[depth_buf_idx] = zp;
//						fragment_shader_payload payload
//						(interpolated_color, interpolated_normal, interpolated_texcoords,
//							texture.has_value() ? &texture.value() : nullptr);
//						payload.view_pos = interpolated_shadingcoords;
//						// 调用你写的片段着色器，拿到最终像素颜色
//						Eigen::Vector3f accum_color;
//						color_accum += fragment_shader(payload);
//						inside_count++;
//					}
//
//				}
//				if (inside_count > 0)
//				{
//					Eigen::Vector3f fin_color = color_accum / 4;
//					Eigen::Vector2i point_pos(x, y);
//					set_pixel(point_pos, fin_color);
//				}
//			}
//		}
//	}
...
//Screen space rasterization
//t是三角形的屏幕空间坐标，view_pos是视图空间顶点位置
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos)
{
	auto v = t.toVector4();
	// 计算三角形的包围盒
	...
	for (int x = (int)x_min; x <= (int)x_max; x++)
	{
		for (int y = (int)y_min; y <= (int)y_max; y++)
		{
			...
			if (insideTriangle(sx, sy, t.v))
			{
				...
				// 获取像素索引（深度缓冲区不再需要4倍扩展）
				int pixel_idx = get_index(x, y);

				// 深度测试：只有当前深度更小（更靠近相机）时才更新
...
rst::rasterizer::rasterizer(int w, int h) : width(w), height(h)
{
    ...
    depth_buf.resize(w * h * 4 , std::numeric_limits<float>::infinity());
    texture = std::nullopt;
}
